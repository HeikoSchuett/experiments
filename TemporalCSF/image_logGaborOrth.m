function image = image_logGaborOrth(imSize,degSize,freq,contrast,center,orientation,L,bw,phase,antiAliasing)
...
% size (rad in deg of visual angle) in the center of the image
%
...
grating = exp(-log2(f./freq.*abs(cos(orient))).^2./2./(0.424.*bw(1)).^2);
zw = -log2(abs(cos(bw(2)))).^2./2./(0.424*bw(1)).^2-2*log(0.5);
eta = freq.*sin(bw(2)).*sqrt(1./zw);
orthf = exp(-(f.*sin(orient)).^2./2./eta.^2);
complexGabor = fftshift(ifft2(ifftshift(grating.*orthf)));
...
if ~useGPU % uses some kind of antialiasing of which I do not know how
%much impact it has. Thus only integer shifts...
    Gabor = imtranslate(Gabor,round(shiftPix));
else
    if shiftPix(1) > 0
